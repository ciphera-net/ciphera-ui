...
export interface ProfileSettingsProps {
  ...
  deriveAuthKey: (password: string, email: string) => Promise<string>
...
export default function ProfileSettings({
  ...
  logout,
  renderPreferences,
  notificationOptions = DEFAULT_NOTIFICATION_OPTIONS
  ...
  useEffect(() => {
    if (user?.preferences?.email_notifications) {
      ...
      // Initialize default state from options if user prefs are empty
      const defaults = notificationOptions.reduce((acc, option) => ({
        ...acc,
        [option.key]: true // Default to true or whatever logic
      }), {})
      setEmailNotifications(defaults)
    }
  }, [user, notificationOptions])
  ...
  const handleUpdateProfile = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await onUpdateProfile(email, currentDerivedKey, newDerivedKey)
  ...
  const handleUpdatePassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await onUpdatePassword(currentDerivedKey, newDerivedKey)
  ...
  const handleDeleteAccount = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Delete all files first (clean up resources) - Optional
      if (onDeleteAllFiles) {
        try {
      ...
      // 3. Delete account
      await onDeleteAccount(derivedKey)
  ...
  const handleVerify2FA = async (e: React.FormEvent) => {
    ...
    setLoading2FA(true)
    try {
      const res = await onVerify2FA(twoFACode)
  ...
  const handleRegenerateCodes = async () => {
    setLoading2FA(true)
    try {
        const res = await onRegenerateRecoveryCodes()
        setRecoveryCodes(res.recovery_codes)
        setShowRecoveryCodes(true)
            ...
            {activeTab === 'preferences' && (
              <div className="space-y-8">
                <div>
                  ...
                  <p className="text-sm text-neutral-500 dark:text-neutral-400">Customize how the app looks on your device.</p>
