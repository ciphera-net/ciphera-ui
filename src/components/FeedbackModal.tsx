...
export interface FeedbackModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit?: (data: { message: string, captcha_id: string, captcha_solution: string, captcha_token: string }) => Promise<void>;
  apiUrl?: string;
}

export default function FeedbackModal({ isOpen, onClose, onSubmit, apiUrl }: FeedbackModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (onSubmit) {
        ...
        const endpoint = apiUrl || (typeof window !== 'undefined' && (window as any).process?.env?.NEXT_PUBLIC_API_URL) || 'http://localhost:8080'
        const response = await fetch(`${endpoint}/api/v1/feedback`, {
          method: 'POST',
          headers: {
